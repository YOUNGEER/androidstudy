package com.wy.younger.animator

import android.animation.ValueAnimator
import android.content.Context
import android.graphics.Canvas
import android.graphics.Color
import android.graphics.Paint
import android.util.AttributeSet
import android.view.View

/**
 * @package:com.learn.android.animator
 * @data on:2019/1/3 13:55
 * author:YOUNG
 * desc: 既改变circle的半径，又动态改变circle的颜色,
 * ValueAnimator.ofObject + TypeEvaluator
 */
class AnimatorView3 : View {
    var mPaint: Paint? = null
    var mValueAnimator: ValueAnimator? = null
    var mRadius: Float = 100f

    constructor(context: Context) : super(context) {
    }

    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) {
        initView()
    }

    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs, defStyleAttr) {
        initView()
    }

    private fun initView() {
        mPaint = Paint()
        mPaint?.color = Color.RED
        mPaint?.style = Paint.Style.FILL
        val startBall = Ball()
        startBall.radius = 50f
        startBall.color = 0xff94E1F7.toInt()

        val endBall = Ball()
        endBall.radius = 150f
        endBall.color = 0xffF35519.toInt()

        mValueAnimator = ValueAnimator.ofObject(BallEvaluate(), startBall, endBall)
        mValueAnimator?.repeatCount = 20
        mValueAnimator?.duration = 1000
        mValueAnimator?.repeatMode = ValueAnimator.REVERSE
        mValueAnimator?.addUpdateListener {

            val ball = it.animatedValue as Ball

            mPaint?.color = ball.color
            mRadius = ball.radius


            invalidate()
        }


        mValueAnimator?.start()


    }

    override fun onDraw(canvas: Canvas?) {
        super.onDraw(canvas)
        canvas?.translate(300f, 300f)
        canvas?.drawCircle(0f, 0f, mRadius, mPaint)

    }


}
